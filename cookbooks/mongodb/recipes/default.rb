...
when "i686"
  ...
  if (@node[:instance_role] == 'util' && @node[:name].match(/mongodb/)) || (@node[:instance_role] == "app_master" &&  @node[:mongo_utility_instances].length == 0)
...
#    include_recipe "mongodb::backup"
  ...
  if (['db_master','solo', 'app_master'].include?(@node[:instance_role]) &&  @node[:mongo_utility_instances].length == 2)
    Chef::Log.info "Setting up Mongo in db_master or solo or app_master"
    include_recipe "mongodb::install"
    include_recipe "mongodb::configure"
#    include_recipe "mongodb::backup"
    include_recipe "mongodb::start"
...
if ['db_master', 'solo', 'app_master'].include? @node[:instance_role]
